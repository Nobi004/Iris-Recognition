...
def get_files1(file_dir):
    ...
    for i in label:
        ...
        print('There are %d '%(len(image_list_train)))
        print('There are %d '%(len(image_list_test)))
        temp= np.array([image_list_test,label_list_test])
        temp= temp.transpose()
        np.random.shuffle(temp)
...
def main():
    file_dir = 'D:\Dataset\iris\CASIA-Iris-Thousand'
    ...
    train.to_csv("D:\Dataset\iris/csv/train.csv",index=False,encoding='utf-8')
    test.to_csv('D:\Dataset\iris/csv/test.csv',index=False,encoding='utf-8')
    print('Finish!')
