...
def MatrixToImage(data):
    #MatrixToImage(data) in Python converts a NumPy matrix (data)
    # into a PIL (Python Imaging Library) image object.

    data = data *255
    #This line scales the intensity values in the data matrix

    new_im = Image.fromarray(data.astype(np.uint8))
    return new_im
    #The function returns the newly created PIL image object.

def ImageEnhancement(normalized_iris):
    row = 64
    col = 512
    normalized_iris = normalized_iris.astype(np.uint8) #(unsigned 8-bit integer)

    enhanced_image = normalized_iris
    enhanced_image = equalize(enhanced_image,disk(32))
    #This line applies an image enhancement technique called histogram equalization.
    #Here,equalize is a function from skimage.filters.rank library
    #also disk is a function from skimage.morphology
...
if __name__== "__main__":
    ...
    for i in range(23,len(train_image_list)):
        eye = train_image_list[i]
        print(i)
